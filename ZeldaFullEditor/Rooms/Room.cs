...
namespace ZeldaFullEditor
{
	...
	public class Room : ICloneable
	{
		...
		public Room(int index, string fromExported = "")
		{
			...
			isdamagePit();
			//palette
			byte aux1 = ROM.DATA[Constants.dungeons_palettes_groups + (palette * 4) + 1];
			byte aux2 = ROM.DATA[Constants.dungeons_palettes_groups + (palette * 4) + 2];
			byte aux3 = ROM.DATA[Constants.dungeons_palettes_groups + (palette * 4) + 3];
			...
			if (data_pointer >= Constants.customCollisionDataPosition)
			{
				...
				while (ROM.ReadShort(Utils.SnesToPc(data_pointer)) != 0x00FFFF)
				{
					...
					while (i < height)
					{
						...
						while (j < width)
						{
							collisionMap[(offset + j + (i * 64))] = ROM.ReadByte(Utils.SnesToPc(data_pointer));
							data_pointer += 1;
