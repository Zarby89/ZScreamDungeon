...
namespace ZeldaFullEditor
{
    ...
    public partial class DungeonMain : Form
    {
        ...
        public DungeonMain()
        {
            ...
            editorsTabControl.Controls[0].BackColor = Color.FromArgb(45, 45, 48);
        ...
        */
        ...
        public void SaveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (this.saveSettingsArr[8])
                {
                    if (save.SaveEntrances(DungeonsData.Entrances, DungeonsData.StartingEntrances))
                    {
                ...
                //DO NOT MOVE, this needs to be before 45.
                if (this.saveSettingsArr[46])
                {
            ...
            // TODO: Why is this part outside of the rest of the save checks?
        ...
        */
        ...
        public void saveLayout(bool clipboard = true)
        {
            ...
            {
                foreach (IClipboardable o in this.activeScene.room.selectedObject)
                {
                    if (this.selectedLayer >= 0)
                    {
                        data.Add(o.MakeClipboardItem());
                    }
                    else if (this.spritemodeButton.Checked)
                    {
                        data.Add(o.MakeClipboardItem());
...
#pragma warning restore SYSLIB0011
            ...
            using (SaveFileDialog saveFile = new SaveFileDialog())
            {
                if (saveFile.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (Room room in opened_rooms)
                    {
                        ...
                        binaryWriter.Write((byte)room.holewarp_plane);
                        ...
                        binaryWriter.Write((byte)room.spriteset);
                        binaryWriter.Write((bool)room.sortsprites);
                        binaryWriter.Write((byte)room.palette);
                        binaryWriter.Write((byte)room.messageid);
                        binaryWriter.Write((byte)room.effect);
                        binaryWriter.Write((byte)room.floor1);
                        binaryWriter.Write((byte)room.floor2);
                        binaryWriter.Write((byte)room.collision);
                        binaryWriter.Write(room.light);
                        binaryWriter.Write((byte)room.blockset);
                        binaryWriter.Write((byte)room.layout);
                    }
        ...
        {
            // IF DARK THEME
            /*e.Graphics.FillRectangle(new SolidBrush(Color.FromArgb(45, 45, 48)), new Rectangle(0,0,this.Width,64));
            foreach(TabPage page2 in editorsTabControl.TabPages)
            {
                TextRenderer.DrawText(e.Graphics, page2.Text, page2.Font, page2.Bounds, Color.White, TextFormatFlags.VerticalCenter | TextFormatFlags.HorizontalCenter);
            }   
