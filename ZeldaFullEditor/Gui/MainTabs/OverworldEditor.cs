...
namespace ZeldaFullEditor.Gui
{
	public partial class OverworldEditor : UserControl
	{
		...
		public void SendMapProperties(OverworldMap map)
		{
			...
			buffer.Write(NetZS.userID); // user ID
			buffer.Write(map.Index);
			buffer.Write(map.AuxPalette);
			buffer.Write(map.GFX);
			buffer.Write(map.MessageID);

			if (map.Index >= 64)
			{
				buffer.Write((byte) 0);
				buffer.Write(map.SpriteGFX[0]);
				buffer.Write(map.SpritePalette[0]);
			}
			else
			{
				buffer.Write(scene.ow.GameState);
				buffer.Write(map.SpriteGFX[this.scene.ow.GameState]);
				buffer.Write(map.SpritePalette[this.scene.ow.GameState]);
			}
